...
const OutfitStack: React.FC<OutfitStackProps> = ({ outfitHistory, onRemoveLastGarment }) => {
  return (
    <div className="flex flex-col">
      <h2 className="text-xl font-serif tracking-wider text-gray-800 border-b border-gray-400/50 pb-2 mb-3">Óculos Ativos</h2>
      <div className="space-y-2">
          ...
          >
            <div className="flex items-center overflow-hidden">
                ...
                <span className="font-semibold text-gray-800 truncate" title={layer.garment?.name}>
                  {layer.garment ? layer.garment.name : 'Modelo de Base'}
                </span>
            </div>
        ...
        {outfitHistory.length === 1 && (
            <p className="text-center text-sm text-gray-500 pt-4">Nenhum óculos selecionado. Escolha um par na lista abaixo para experimentar.</p>
        )}
      </div>
    </div>
  );
